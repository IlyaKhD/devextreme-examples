...
import Map, {
    ...
    Title,
    Tooltip
} from 'devextreme-react/ui/vector-map';
...
function onTooltipShown(e) {
    ...
    const gdpData = countriesGDP[name] ? [{
        name: name,
        agriculture: countriesGDP[name].agriculture,
        industry: countriesGDP[name].industry,
        services: countriesGDP[name].services
    }] : [{ name: name }]
    const container = document.getElementById('gdp-sectors');
    if (gdpData[0].services) {
        new DxChart(container, getChartConfig(gdpData));
    } else {
